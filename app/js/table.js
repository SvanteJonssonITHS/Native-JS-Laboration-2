...
prepareTableData = (country) => {
	const data = []
	if (country.region) data.push({ label: 'Continent', value: country.region })
	...
	if (country.demonyms) data.push({ label: 'Denonyms', value: Object.values(country.demonyms.eng) }) // add male and female seperators later
	if (country.gini) data.push({ label: 'Gini', value: Object.values(country.gini) }) // Only show the latest
	if (country.currencies) data.push({ label: 'Currency', value: Object.values(country.currencies) }) //fix
	...
	if (country.car.signs) data.push({ label: 'License plate', value: country.car.signs })
